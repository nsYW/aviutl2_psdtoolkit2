...
static void section_to_internal_section_name_n(char const *const section,
...
static struct section const *find_section(struct ptk_ini_reader const *const reader, char const *const section) {
  if (!reader) {
    return NULL;
  }
...
static struct section const *
find_section_n(struct ptk_ini_reader const *const reader, char const *const section, size_t const section_len) {
  if (!reader) {
    return NULL;
  }
  char const *internal_name;
  size_t internal_len;
  section_to_internal_section_name_n(section, section_len, &internal_name, &internal_len);
  return (struct section const *)OV_HASHMAP_GET(reader->sections,
                                                &((struct section const){
                                                    .name = internal_name,
                                                    .name_len = internal_len,
                                                }));
}
...
void ptk_ini_reader_destroy(struct ptk_ini_reader **const rp) {
  ...
  struct ptk_ini_reader *const r = *rp;
  if (r->sections) {
...
bool ptk_ini_reader_create(struct ptk_ini_reader **const rp, struct ov_error *const err) {
  if (!rp || *rp) {
  ...
  bool result = false;
  struct ptk_ini_reader *r = NULL;

  if (!OV_REALLOC(&r, 1, sizeof(struct ptk_ini_reader))) {
    OV_ERROR_SET_GENERIC(err, ov_error_generic_out_of_memory);
    goto cleanup;
  }
  *r = (struct ptk_ini_reader){
...
static struct ov_hashmap *get_or_create_section_entries(struct ptk_ini_reader *const r,
                                                        char const *const section,
                                                        size_t section_len,
                                                        size_t line_number,
...
static bool parse(struct ptk_ini_reader *const reader,
...
bool ptk_ini_reader_load(struct ptk_ini_reader *const reader,
...
bool ptk_ini_reader_load_file(struct ptk_ini_reader *const reader,
  ...
  if (!ptk_ini_reader_load(reader, source, err)) {
    OV_ERROR_ADD_TRACE(err);
    goto cleanup;
  }
...
bool ptk_ini_reader_load_memory(struct ptk_ini_reader *const r,
  ...
  if (!ptk_ini_reader_load(r, source, err)) {
    OV_ERROR_ADD_TRACE(err);
    goto cleanup;
  }
...
static struct ptk_ini_value extract_value_from_line(char const *const line, size_t const line_len) {
  struct ptk_ini_value result = {NULL, 0};
...
ptk_ini_reader_get_value(struct ptk_ini_reader const *const reader, char const *const section, char const *const key) {
  struct ptk_ini_value result = {NULL, 0};
  if (!reader || !key) {
    goto cleanup;
  }
...
struct ptk_ini_value ptk_ini_reader_get_value_n(struct ptk_ini_reader const *const reader,
                                                char const *const section,
                                                size_t const section_len,
                                                char const *const key,
                                                size_t const key_len) {
  ...
  {
    struct section const *const s = find_section_n(reader, section, section_len);
    if (!s) {
      goto cleanup; // section not found
    }
    struct entry const *const e = (struct entry const *)OV_HASHMAP_GET(s->entries,
...
bool ptk_ini_reader_iter_sections(struct ptk_ini_reader const *const reader, struct ptk_ini_iter *const iter) {
  if (!reader || !iter) {
    return false;
  }
...
bool ptk_ini_reader_iter_entries(struct ptk_ini_reader const *const reader,
                                 char const *const section,
...
bool ptk_ini_reader_iter_entries_n(struct ptk_ini_reader const *const reader,
                                   char const *const section,
                                   size_t const section_len,
                                   struct ptk_ini_iter *const iter) {
  if (!reader || !iter) {
  ...
  // First call or section not cached - find the section
  if (!s) {
    s = find_section_n(reader, section, section_len);
    if (!s) {
      return false;
    }
    iter->state = s;
...
size_t ptk_ini_reader_get_section_count(struct ptk_ini_reader const *const reader) {
  if (!reader) {
    return 0;
  }
...
size_t ptk_ini_reader_get_entry_count(struct ptk_ini_reader const *const reader, char const *const section) {
  if (!reader) {
    return 0;
  }
